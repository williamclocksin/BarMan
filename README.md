...
innovations into music notation editing, though the user base was relatively small and development stopped
when NeXTStep folded.  The first release of Calliope was written in Objective-C.
A more recent project ported Calliope to Java and JavaFX,and the most recent version (still with bugs) ported some of the Java code to Scala in order to experiment with a composition-based data dtructure model.  This experiment was abandoned because Scala seems to be an academic project that can't decide what kind of language it is as it tries to be everything. It also runs about 5x slower than the Java version. So, I am trying to simplify things, still with composition data structures, by using TypeScript.  I wish I could continue to use JavaFX for the graphics, but I may need to learn something like React.
